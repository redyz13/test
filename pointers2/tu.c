...
int crea_elenco(Persona **lista, Persona **elenco_con_cognomi, int index, int *cognomi) {
    ...
    for(i = 0; i < index; i++) {
        ...
        if(found != -1) {
            ...
            elenco_con_cognomi[i] = xmalloc(sizeof(Persona));
            elenco_con_cognomi[i]->nome = xmalloc(sizeof(strlen(nome) + 1));
            elenco_con_cognomi[i]->cognome = xmalloc(sizeof(strlen(cognome) + 1));
